...
class  PayplansAppOffline extends PayplansAppPayment
{
	...
	public function _renderForm(PayplansPayment $payment = null, $form='form')
	{
		...
		$transParams->set('amount',	$invoice->getTotal());
		$transParams->set('currency', $invoice->getCurrency('isocode'));
	...
	public function onPayplansPaymentAfter(PayplansPayment $payment, &$action, &$data, $controller)
	{
		...
		$data['transaction']		= 	array_merge($appParameter, $data['transaction']);
		$appData['gateway_params']  = 	PayplansHelperParam::arrayToIni($data['transaction']);
		...
		if($action == 'success'){
			...
			$transaction->set('user_id', $payment->getBuyer())
						...
						->set('amount', 0)
						->set('gateway_txn_id', isset($data['transaction']['id']) ? $data['transaction']['id'] : 0)
